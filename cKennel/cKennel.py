...
def listPets():
        ...
        if len(availableFriends) >= 1 and availableFriends[0] != '': #There is a file, but is there something in it
                print '\n///////////////////////////'
        ...
        while not entryComplete:		
                ...
                while not validInput: #go through and re-prompt if necessary
                        ...
                        if sex == 'm' or sex == 'f': #sex must be either m or f
                                ...
                                sex = raw_input('Sex (m/f):') #re-prompt for (m) or (f)

                #Display what the user entered to them for confirmation
                print '\nYou entered:'
                print 'Name: {0}\nrace: {1}\nSex: {2}\nAge: {3}\nBreed: {4}\nNote: {5}\n'.format(name,race,sex,age,breed,note)

                confirmEntry = raw_input('Is the entered information correct?(y/n):') #Is everything correct

                #prep animal for admittance
                if confirmEntry == 'y':

                        newFriend = ':'.join([name,race,sex,age,breed,note]) #join the elements together like name:race:sex:age:breed:note
                        ...
                        try:		
                                ...
                                print 'File write error({0}): {1}'.format(e.errno, e.strerror)
